...
export const getRecepcionAction = async (params?: PaginationParams) => {
  const queryParams: Record<string, number> = {};
  if (params?.limit !== undefined) queryParams.limit = params.limit;
  if (params?.offset !== undefined) queryParams.offset = params.offset;

  const { data } = await RecepcionApi.get<
    Recepcion[] | PaginatedResponse<Recepcion>
  >('/', {
  ...
  if (
    ...
    const paged = data as PaginatedResponse<Recepcion>;
    const pageData = paged.data || [];
    // Ordenar por fecha de recepción DESC (más recientes primero)
    pageData.sort((a, b) => {
    ...
    return {
      ...
      limit: limitValue,
      offset: offsetValue,
    } as PaginatedResponse<Recepcion>;
  }
