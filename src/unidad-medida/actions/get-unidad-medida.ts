...
): Promise<UnidadMedida[] | PaginatedResponse<UnidadMedida>> => {
  try {
    ...
    const response = await UnidadMedidaApi.get<any>('/', {
      params: queryParams,
    });
    ...
    if (params?.limit !== undefined || params?.offset !== undefined) {
      ...
      if (Array.isArray(response.data)) {
        ...
        try {
          const totalResponse = await UnidadMedidaApi.get<any>('/', {
            params: {
          ...
          if (Array.isArray(totalResponse.data)) {
            allUnidades = totalResponse.data;
          } else if (
            totalResponse.data &&
            typeof totalResponse.data === 'object' &&
            'data' in totalResponse.data
          ) {
            allUnidades = (totalResponse.data as any).data || [];
          }
          
          // Normalizar, filtrar solo activos y ordenar
          const allNormalized = allUnidades.map(normalize);
