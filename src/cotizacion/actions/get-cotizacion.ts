...
export const getCotizacionesAction = async (
  params?: PaginationParams
): Promise<Cotizacion[] | PaginatedResponse<Cotizacion>> => {
  try {
    ...
    const response = await cotizacionApi.get<any>('/', {
      params: queryParams,
    });
    ...
    if (params?.limit !== undefined || params?.offset !== undefined) {
      ...
      if (Array.isArray(response.data)) {
        ...
        try {
          const totalResponse = await cotizacionApi.get<any>('/', {
            params: {
          ...
          if (Array.isArray(totalResponse.data)) {
            allCotizaciones = totalResponse.data;
          } else if (
            totalResponse.data &&
            typeof totalResponse.data === 'object' &&
            'data' in totalResponse.data
          ) {
            allCotizaciones = (totalResponse.data as any).data || [];
          }
          
          // Ordenar todos
          allCotizaciones.sort((a, b) => {
            const dateA = new Date(a.fecha || 0).getTime();
            const dateB = new Date(b.fecha || 0).getTime();
            return dateB - dateA; // DESC
          });
          
          const total = allCotizaciones.length;
          
          // Ordenar la página actual
          const currentData = response.data as Cotizacion[];
      ...
      if (
        ...
        const allData = (response.data.data || []) as Cotizacion[];
    ...
    if (Array.isArray(response.data)) {
      const allData = response.data as Cotizacion[];
      allData.sort((a, b) => {
        const dateA = new Date(a.fecha || 0).getTime();
        const dateB = new Date(b.fecha || 0).getTime();
        return dateB - dateA; // DESC
