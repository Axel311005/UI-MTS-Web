...
import { Plus, Search, Filter, Eye, Edit, Trash2, Coins } from '@/shared/icons';
import {
  Card,
  CardContent,
  CardHeader,
...
export function MonedaPage() {
  ...
  const { monedas, totalItems = 0 } = useMoneda({
    usePagination: true,
    limit,
    offset,
  });

  const filteredMonedas = useMemo<Moneda[]>(() => {
    const items = monedas ?? [];
    const term = searchTerm.trim().toLowerCase();
    if (!term) return items;

    return items.filter((moneda) => {
      const descripcion = moneda.descripcion?.toLowerCase() ?? '';
  ...
  const handleDelete = async (id: number) => {
    try {
      await patchMoneda(id, { activo: EstadoActivo.INACTIVO });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-left">
            Gestiona las monedas para tus transacciones
          </p>
        </div>
        <Button
            ...
            <TableBody>
              {filteredMonedas.length === 0 ? (
                ...
                filteredMonedas.map((moneda) => (
                  <TableRow key={moneda.idMoneda} className="table-row-hover">
                    ...
                    <TableCell>
                      {Number(moneda.tipoCambio).toLocaleString('es-PY', {
                    ...
                    <TableCell>
                      <Badge
                        variant={
                          moneda.activo === EstadoActivo.ACTIVO
                            ? 'default'
                            : 'secondary'
                        }
                      >
                    ...
                    <TableCell>{formatDate(moneda.fechaCreacion)}</TableCell>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem
                            ...
                            onClick={() => handleDelete(moneda.idMoneda)}
                          >
