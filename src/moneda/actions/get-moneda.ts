...
): Promise<Moneda[] | PaginatedResponse<Moneda>> => {
  try {
    ...
    const response = await monedaApi.get<any>('/', {
      params: queryParams,
    });
    ...
    if (params?.limit !== undefined || params?.offset !== undefined) {
      ...
      if (Array.isArray(response.data)) {
        ...
        try {
          const totalResponse = await monedaApi.get<any>('/', {
            params: {
          ...
          if (Array.isArray(totalResponse.data)) {
            allMonedas = totalResponse.data;
          } else if (
            totalResponse.data &&
            typeof totalResponse.data === 'object' &&
            'data' in totalResponse.data
          ) {
            allMonedas = (totalResponse.data as any).data || [];
          }
          
          // Normalizar, filtrar solo activos y ordenar
          const allNormalized = allMonedas.map(normalize);
