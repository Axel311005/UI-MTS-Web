...
export const getProformaAction = async (
  params?: PaginationParams
): Promise<Proforma[] | PaginatedResponse<Proforma>> => {
  try {
    ...
    const response = await ProformaApi.get<any>('/', {
      params: queryParams,
    });
    ...
    if (params?.limit !== undefined || params?.offset !== undefined) {
      ...
      if (Array.isArray(response.data)) {
        ...
        try {
          const totalResponse = await ProformaApi.get<any>('/', {
            params: {
          ...
          let allProformas: Proforma[] = [];
          if (Array.isArray(totalResponse.data)) {
            allProformas = totalResponse.data;
          } else if (
            totalResponse.data &&
            typeof totalResponse.data === 'object' &&
            'data' in totalResponse.data
          ) {
            allProformas = (totalResponse.data as any).data || [];
          }
          ...
          const total = allProformas.length;
          
          // Ordenar la página actual
          const currentData = response.data as Proforma[];
      ...
      if (
        ...
        const allData = (response.data.data || []) as Proforma[];
    ...
    if (Array.isArray(response.data)) {
      const allData = response.data as Proforma[];
      allData.sort((a, b) => {
        const dateA = new Date(a.fecha || 0).getTime();
        const dateB = new Date(b.fecha || 0).getTime();
        return dateB - dateA; // DESC
