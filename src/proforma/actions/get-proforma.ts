...
export const getProformaAction = async (params?: PaginationParams) => {
  const queryParams: Record<string, number> = {};
  if (params?.limit !== undefined) queryParams.limit = params.limit;
  if (params?.offset !== undefined) queryParams.offset = params.offset;

  const { data } = await ProformaApi.get<
    Proforma[] | PaginatedResponse<Proforma>
  >('/', {
  ...
  if (
    ...
    const paged = data as PaginatedResponse<Proforma>;
